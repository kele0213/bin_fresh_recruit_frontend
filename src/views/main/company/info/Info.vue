...
const info = ref([])
const label = ["企业名称", "手机号", "企业介绍", "企业地址", "企业规模", "企业类型", "工作时间", "成立时间", "企业福利"]
...
onMounted(async () => {
  ...
  // 封装数据
  watch(() => companyInfo.value, async (newValue) => {
    info.value = []
...
<template>
  ...
  <modal-form :form-config="updateConfig" ref="updateRef" :data="companyInfo" @confirm="update"></modal-form>
</template>

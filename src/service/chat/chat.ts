...
// 企业图片发送
export function addChatByPictureByComHttp(data: FormData) {
    return httpService.post<Idata<ChatVo>>({
        url: '/chat/com/picture',
        headers: {
...
// 应届生图片发送
export function addChatByPictureByFreshHttp(data: FormData) {
    return httpService.post<Idata<ChatVo>>({
        url: '/chat/fresh/picture',
        headers: {
            'Content-Type': 'multipart/form-data'
        },
        data
